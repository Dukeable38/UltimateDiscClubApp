const express = require('express');
     ...
     router.post('/signup', async (req, res) => {
       ...
       try {
         ...
         const result = await pool.query(
           'INSERT INTO players (name, email, password, student_number, gender, skill, class, payment) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id, name, email, student_number, gender, skill, class, payment',
           [name, email, hashedPassword, student_number, gender || 'other', 3, 'Novice', 'Weekly']
         );
         const token = jwt.sign({ player_id: result.rows[0].id }, 'your_jwt_secret', { expiresIn: '1h' });
     ...
     router.post('/login', async (req, res) => {
       ...
       try {
         ...
         const token = jwt.sign({ player_id: player.id }, 'your_jwt_secret', { expiresIn: '1h' });
         res.json({ 
           token, 
           player: { 
             id: player.id, 
